      ...
      <table id="result_list" class="table table-striped table-bordered table-hover table-condensed">
        ...
        {% for item, item_perms in paginated_items.object_list %}
            ...
            <tr class="{% cycle rowcolors %}">
                ...
                {% endif %}
                <!-- FileIcon -->
                <td>{% if item_perms.change %}<a href="/media/{{ file.file }}" title="{% blocktrans with file.label as item_label %}Change '{{ item_label }}' details{% endblocktrans %}">{% endif %}<img src="{% if file.icons.48 %}{{ file.icons.48 }}{% else %}{% filer_staticmedia_prefix %}icons/missingfile_48x48.png{% endif %}" alt="{{ file.default_alt_text }}" />{% if item_perms.change %}</a>{% endif %}</td>
                <!-- Filename/Dimensions -->
                <td>
                   {% if user.is_superuser %}
                    ...
                    {% endif %}
                    <div><b><a href="/media/{{ file.file }}{% if is_popup %}?_popup=1{% if select_folder %}&select_folder=1{% endif %}{% endif %}" title="{% blocktrans with file.label as item_label %}Change '{{ item_label }}' details{% endblocktrans %}">{{ file.label }}</a></b></div>
                    <div class="file-info muted"><span class="file-size">{{ file.size|filesizeformat }}{% ifequal file.file_type "Image" %}, {{ file.width }}x{{ file.height }} px{% endifequal %}</span><br>{% trans "Owner" %}: {{ file.owner|default:"n/a" }}{% if enable_permissions %}. {% trans "Permissions" %}: {% if file.is_public %}{% trans "disabled" %}{% else %}{% trans "enabled" %}{% endif %}{% endif %}</div>
