...
{% block content %}
  <div id="content-main">
    <img src="http://104.236.142.130:8000/media/filer_public/14/6e/146ef58a-7e76-4a7d-8a11-a815c90e7b63/logo_05.png" width="55%" style="display:block;" />
    {% if app_list %}
      {% for app in app_list %}
        <div class="module">
          <table class="table-overview applist" summary="{% blocktrans with name=app.name %}Models available in the {{ name }} application.{% endblocktrans %}">
            ...
            {% for model in app.models %}
              <tr>
                ...
                {% if model.admin_url %}
                  <td>
                    <a href="{{ model.admin_url }}" class="changelink icon">{% trans 'Ir a repositorio' %}</a>
                  </td>
                {% else %}
                  <td>&nbsp;</td>
                {% endif %}
  ...
  <script type="text/javascript">
      location.href="/admin/filer/folder/";
  </script>
{% endblock %}
